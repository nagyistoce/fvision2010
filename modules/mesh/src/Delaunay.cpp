...
void Delaunay::Triangulate(const vertexSet& vertices, triangleSet& output)
{
	...
	for (itVertex = vertices.begin(); itVertex != vertices.end(); itVertex++)
	{
		...
		triangleIsCompleted tic(itVertex, output, vSuper);
		triangleSet::iterator it= workset.begin();
		for (; it != workset.end(); ) { if (tic(*it)) workset.erase(it++); else ++it; }

		edgeSet edges;

		// A triangle is 'hot' if the current vertex v is inside the circumcircle.
		// Remove all hot triangles, but keep their edges.
		//itEnd = remove_if(workset.begin(), it, vertexIsInCircumCircle(itVertex, edges));
		vertexIsInCircumCircle vic(itVertex, edges);
		triangleSet::iterator it2= workset.begin();
		for (; it2 != it; ) { if (tic(*it2)) workset.erase(it2++); else ++it2; }
		triangleSet::iterator itEnd = it2;
